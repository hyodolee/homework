...
public class MainApp {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i < full.length; i++) {
			System.out.println(full[i].getEmpNo() + " | " + full[i].geteName()+ " | " + full[i].getJob()+ " | " + full[i].getMgr()+ " | " + full[i].getHiredate()+ " | " + full[i].getDepName()+ " | " + full[i].getSalary()+ " | " + full[i].getBonus());
		}
		
		System.out.println("*******************PartTime 정보**********************************");
		for(int i=0; i < part.length; i++) {
			System.out.println(part[i].getEmpNo() + " | " + part[i].geteName()+ " | " + part[i].getJob()+ " | " + part[i].getMgr()+ " | " + part[i].getHiredate()+ " | " + part[i].getDepName()+ " | " + part[i].getTimePay());
		}
		
		System.out.println("*******************Message**********************************");
		for(int i=0; i < full.length; i++) {
			System.out.println(full[i].geteName()+ "사원은 정규직 입니다.");
		}
		for(int i=0; i < part.length; i++) {
			System.out.println( part[i].geteName()+ "사원은 비정규직 입니다." );
		}
	}
